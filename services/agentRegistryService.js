...
const STATIC_AGENTS = [
  {
    ...
    connections: [
      ...
      { agentId: 'agentFactoryAgent', name: 'Agent Factory', nameHe: 'מפעל סוכנים', direction: 'outgoing', type: 'dispatch', payload: 'intent=factory', trigger: 'כל הודעה', risk: 'high', requiresApproval: true, confidence: 'inferred' },
    ],
  ...
  {
    id: 'agentFactoryAgent',
    name: 'Agent Factory',
    nameHe: 'מפעל סוכנים',
    role: 'יצירת סוכנים דינמיים',
    mission: 'יוצר סוכנים מותאמים אישית בזמן ריצה. כותב קבצי JS ל-agents/custom/ ומעדכן את registry.json. הסוכנים נטענים דינמית.',
    prompt: 'צור סוכן חדש לפי הדרישות הבאות. כתוב קוד JavaScript תקין. רשום ב-registry.',
    responsibilities: ['יצירת קבצי JS', 'עדכון registry.json', 'hot-loading סוכנים'],
    inputs: ['תיאור סוכן + דרישות'],
    outputs: ['קובץ JS חדש + registry מעודכן'],
    tools: ['fs (כתיבת קבצים)', 'callGemma4 (יצירת קוד)'],
    permissions: ['כתיבה ל-agents/custom/', 'עדכון registry.json'],
    memoryAccess: 'אין',
    restrictions: ['קוד נבדק לפני טעינה', 'לא מוחק סוכנים קיימים'],
    risk: 'high', mode: 'autonomous', approval: 'high', autonomy: 60, status: 'active',
    connections: [
      { agentId: 'router', name: 'Router', nameHe: 'ראוטר', direction: 'outgoing', type: 'רישום keywords', payload: 'registry.json → router keywords', trigger: 'אחרי יצירת סוכן', risk: 'high', requiresApproval: true, confidence: 'inferred' },
    ],
    dashboard: { tasksHandled: 'unknown', failures: 'unknown', avgLatency: 'unknown', confidence: 'unknown', lastActive: 'unknown' },
  },
  {
    ...
    nameHe: 'סוכן סקרים',
    role: 'ניהול סקרים',
    mission: 'יוצר ומנהל סקרים ושאלונים. שומר תשובות ב-Supabase user_surveys.',
    prompt: 'נהל סקרים: צור, שלח, אסוף תשובות.',
    responsibilities: ['יצירת סקרים', 'איסוף תשובות'],
    inputs: ['הודעת משתמש'],
    outputs: ['{ answer, action }'],
    tools: ['Supabase user_surveys', 'callGemma4'],
    permissions: ['קריאה/כתיבה ב-user_surveys'],
    memoryAccess: 'אין',
    restrictions: [],
    risk: 'low', mode: 'operator', approval: 'none', autonomy: 30, status: 'active',
    connections: [],
    dashboard: { tasksHandled: 'unknown', failures: 'unknown', avgLatency: 'unknown', confidence: 'unknown', lastActive: 'unknown' },
  },
];
