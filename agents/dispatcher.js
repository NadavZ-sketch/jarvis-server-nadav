...
const REGISTRY = {
    ...
    shopping:   { mode:'sync', invoke:(c,a) => a.runShoppingAgent(c.userMessage, c.repos, c.useLocal) },
    notes:      { mode:'sync', invoke:(c,a) => a.runNotesAgent(c.userMessage, c.repos, c.useLocal) },
    habit:      { mode:'sync', invoke:(c,a) => a.runHabitAgent(c.userMessage, c.repos, c.useLocal, c.settings) },
    insight:    { mode:'sync', invoke:(c,a) => a.runInsightAgent(c.userMessage, c.repos, c.useLocal, c.settings) },
    stocks:     { mode:'sync', invoke:(c,a) => a.runStocksAgent(c.userMessage) },
    translate:  { mode:'sync', invoke:(c,a) => a.runTranslationAgent(c.userMessage, c.useLocal) },
    music:      { mode:'sync', invoke:(c,a) => a.runMusicAgent(c.userMessage, c.repos, c.useLocal, c.settings) },
    sports:     { mode:'sync', invoke:(c,a) => a.runSportsAgent(c.userMessage) },
    messaging:  { mode:'sync', invoke:(c,a) => a.runMessagingAgent(c.userMessage, c.repos, c.useLocal, c.settings) },
    draft:      { mode:'sync', invoke:(c,a) => a.runDraftAgent(c.userMessage, c.chatHistory, c.longTermMemories, c.settings) },
    calendar:   { mode:'sync', invoke:(c,a) => a.runCalendarAgent(c.userMessage, c.repos, c.settings) },
    prompt:     { mode:'sync', invoke:(c,a) => a.runPromptAgent(c.userMessage, c.repos, c.useLocal, c.settings) },
    settings:   { mode:'sync', invoke:(c,a) => a.runSettingsAgent(c.userMessage, c.useLocal, c.settings) },
    project:    { mode:'sync', invoke:(c,a) => a.runProjectAgent(c.userMessage, c.repos, c.useLocal, c.settings) },
    factory:    { mode:'sync', invoke:(c,a) => a.runAgentFactoryAgent(c.userMessage, c.repos, c.useLocal, c.settings) },
